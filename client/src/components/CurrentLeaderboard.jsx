...
const CurrentLeaderboard = ({ 
  ...
  useEffect(() => {
    ...
    if (showAnimation) {
      ...
      const startScores = {};
      leaderboard.forEach(entry => {
        const latestPoints = entry.perQuestionScores[entry.perQuestionScores.length - 1]?.pointsEarned || 0;
      ...
      const timer = setInterval(() => {
        ...
        leaderboard.forEach(entry => {
          ...
          newScores[entry.userId] = Math.round(start + (target - start) * progress);
        });
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {leaderboard.map((entry) => {
            const latestScore =
              entry.perQuestionScores[
                entry.perQuestionScores.length - 1
              ];
            const displayScore =
